...
export function Sidebar({ currentChatId, onChatSelect, onNewChat, onDeleteChat, chats, onRefreshChats, isOpen, onClose, isLoadingChats = false }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Ultra Premium Sidebar Container */}
      <aside
        id="sidebar"
        className={cn(
          ...
          'transition-all duration-180 ease-elite',
      ...
      >
        {/* Subtle ambient glow from the right */}
        <div className="absolute top-0 right-0 w-32 h-full bg-gradient-to-l from-[var(--color-accent-500)]/3 to-transparent pointer-events-none" />

        {/* Ultra Premium Collapse Toggle - Desktop only */}
        <div className="hidden lg:block absolute -right-3.5 top-6 z-10">
          <button
            onClick={() => setIsCollapsed(!isCollapsed)}
            className="w-8 h-8 rounded-2xl bg-[var(--color-bg-elevated)]/95 backdrop-blur-md border border-[var(--glass-border)] flex items-center justify-center text-[var(--color-text-tertiary)] hover:text-[var(--color-accent-400)] hover:border-[var(--color-accent-500)]/50 hover:shadow-lg hover:shadow-[var(--color-accent-glow-subtle)] transition-all duration-180 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-500)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)] group"
            aria-label={isCollapsed ? 'Sidebar erweitern' : 'Sidebar minimieren'}
            aria-expanded={isCollapsed}
          >
            <svg
              className={cn('w-4 h-4 transition-transform duration-180 group-hover:rotate-12', isCollapsed ? 'rotate-180' : '')}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        {isCollapsed ? (
          <div className="flex flex-col h-full p-3">
            {/* New Chat Icon - Ultra Premium */}
            <button
              onClick={onNewChat}
              className="group relative flex items-center justify-center w-12 h-12 mb-4 rounded-2xl bg-gradient-to-br from-[var(--color-accent-500)] via-[var(--color-accent-550)] to-[var(--color-accent-600)] text-white shadow-xl shadow-[var(--color-accent-glow-strong)] hover:shadow-2xl hover:shadow-[var(--color-accent-glow-ultra)] hover:scale-110 transition-all duration-180 focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-500)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)] overflow-hidden"
              title="Neuer Chat"
          ...
          <>
            ...
            <nav className="flex-shrink-0 p-5 space-y-2">
              {/* Ultra Premium New Chat Button */}
              <button
                onClick={onNewChat}
                className="group relative w-full flex items-center gap-3 px-5 py-3.5 rounded-2xl bg-gradient-to-r from-[var(--color-accent-500)] via-[var(--color-accent-550)] to-[var(--color-accent-600)] text-white shadow-xl shadow-[var(--color-accent-glow-strong)] hover:shadow-2xl hover:shadow-[var(--color-accent-glow-ultra)] hover:scale-[1.02] transition-all duration-180 overflow-hidden focus:outline-none focus:ring-2 focus:ring-[var(--color-accent-500)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg-primary)]"
              >
                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-700" />
